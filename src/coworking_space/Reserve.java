...
public Reserve(int max,double f){
    this.maxVisitor=max;
    this.fees=f;
...
void reserve_hours(int day,int h,String n){
    if (!check_reserve(day,h))
        {
    schedule[day][h].reserved=true;
    schedule[day][h].getUserName().add(n);
    System.out.println("reservation done successfully to :"+n);
        }
...
boolean check_reserve(int day ,int h){
    ...
    void cansle_reservation(int day,int h,String name_user){
    schedule[day][h].cancel_reservation(name_user);
    }
    void cansle_reservationrby_username(String name ){
